...
public class FontBuilderArm implements FontBuilder {
	@Override
	public CustomFont buildFont(String name, int style, int size) {
		return new CustomFontArm(name, style, size);
	}
	
	@Override
	public CustomFont buildFont(String path) {
		return new CustomFontArm(loadFont(path));
	}
	
	@Override
	public SimpleFont createFontObject() {
		return new SimpleFontArm();
	}
